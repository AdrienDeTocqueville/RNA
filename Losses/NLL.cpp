...
void NLL::toGPU(const cl_context& _context, const cl_device_id& _deviceId)
{
    if (!lossKernel)
        lossKernel = loadKernel(_context, _deviceId, "OpenCL/losses.cl", "mseGradient");

    if (!gradientKernel)
        gradientKernel = loadKernel(_context, _deviceId, "OpenCL/losses.cl", "mseGradient");
}
